/**
 * ============================================================================
 * model.ts — 抖音直播弹幕协议 protobuf 编解码器（手写生成，勿手改结构）
 * ============================================================================
 *
 * 【来源与背景】
 *   本文件由 protobuf 定义生成（pbjs 风格的纯 JS 编解码，无运行时依赖），
 *   包含抖音 webcast 协议的全部消息结构。字段序号（field number）与 wire
 *   format 必须与官方 .proto 完全一致，改动字段会直接导致解码错位/崩溃。
 *
 * 【帧结构总览（与 dycast.ts 配合）】
 *   PushFrame（传输层帧，WSS 收发的都是它）
 *     ├─ seqId / logId / service / method   ：帧元信息（uint64）
 *     ├─ headersList                        ：map<string,string> 帧头
 *     │    ├─ im-cursor          消息游标（ACK 需带回）
 *     │    ├─ im-internal_ext    内部扩展串（ACK payload，含 wss_push_did）
 *     │    └─ compress_type      取值 'gzip' 时 payload 需先解压
 *     └─ payloadType             'hb' | 'ack' | 'msg' | 'close'
 *     └─ payload                 Response 的二进制编码（可能 gzip）
 *
 *   Response（业务层，payload 解压后解码）
 *     ├─ messages[]             每条含 method（消息类型名）+ payload（业务体）
 *     ├─ cursor / fetchInterval / now / internalExt / needAck
 *     └─ pushServer             WSS 推送域名（im/fetch 响应也用它）
 *
 * 【本文件导出的解码函数（供 dycast.ts 按 method 分发）】
 *   decodeChatMessage      弹幕（content + user + rtfContentV2 富文本）
 *   decodeGiftMessage      礼物（gift + toUser + repeatCount/comboCount/repeatEnd）
 *   decodeLikeMessage      点赞（count + total）
 *   decodeMemberMessage    进场（user + memberCount 在线人数）
 *   decodeSocialMessage    关注（user + followCount 粉丝数）
 *   decodeEmojiChatMessage 会员表情弹幕（emojiContent 表情图）
 *   decodeRoomUserSeqMessage 观众席（ranks 送礼榜 + total 在线 + totalUser 累计）
 *   decodeControlMessage   控制消息（action + common.describe）
 *   decodeRoomRankMessage  小时榜（ranks 榜单）
 *   decodeRoomStatsMessage 房间统计（displayMiddle 在线展示值）
 * ============================================================================
 */
