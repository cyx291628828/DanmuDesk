...
import { frontierSign } from './signer';
 ...
 */
export const getSignature = async function (roomId, uniqueId) {
  ...
  // 桌面版：由 signer 提供（隐藏 BrowserWindow 加载抖音页面后注入）
  const res = await frontierSign({
    'X-MS-STUB': e
  });
  return res['X-Bogus'] || '';
};
